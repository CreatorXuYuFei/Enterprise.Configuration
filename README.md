...
# 核心特性
✅ 全兼容原生配置：完全基于微软官方 Configuration 构建，原有代码无需大幅改造即可迁移 /n
✅ 多环境配置隔离：内置 Development/Staging/Production 多环境自动切换规则，支持自定义环境 /n
✅ 配置热更新：监听配置文件变更，自动重载配置，无需重启应用 /n
✅ 配置项加密：支持 JSON/XML 配置文件敏感字段加密，适配数据库连接串、密钥等隐私配置 /n
✅ 增强型强类型绑定：优化实体绑定逻辑，支持嵌套实体、数组、枚举、默认值填充 /n
✅ 多配置源聚合：统一整合 JSON、XML、INI、环境变量、命令行、内存配置、远程配置源 /n
✅ 配置合法性校验：内置数据校验规则，启动时自动检测必填项、数值范围、格式合法性 /n
✅ 分层配置合并：支持全局配置 + 项目私有配置 + 本地开发配置三级合并策略 /n
✅ 轻量无依赖：除 .NET 官方基础包外，无第三方重型依赖，体积小巧、性能高效 /n
